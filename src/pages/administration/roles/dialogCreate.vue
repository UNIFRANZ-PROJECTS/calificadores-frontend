  ...
  methods: {
    ...
    createNewRol() {
      if( this.editedIndex === -1 ){
        this.$http.post("register/rol/permision", {
            rls_name: this.estructure.rls_name,
            ...
            for (let i = 0; i < this.estructure.rls_permisions.length; i++) {
              permisionData.push({
                "serv_permision": {
                    ...
                    "prm_name": this.getPermisions.filter((e)=>e.id == this.estructure.rls_permisions[i])[0].prm_name
                }
              });
            }
        ...
        this.$http.put('/rol/update/'+this.estructure.id, {
          rls_name:this.estructure.rls_name,
          rls_permisions: this.estructure.rls_permisions,
          rls_state:this.estructure.rls_state
        })
        .then((result) => {
          this.show = false;
