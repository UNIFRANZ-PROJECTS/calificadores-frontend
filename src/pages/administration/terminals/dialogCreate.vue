        ...
        <v-container>
          <v-row>
            ...
            <v-col cols="1" sm="12" md="6">
              <v-textarea
                v-model="estructure.ars_description"
                color="cyan"
                rows="3"
                required
                auto-grow=""
                clear-icon="mdi-close-circle-outline"
                @click:clear="clearVariable('descripcion')"
                clearable
                :rules="[
                  (v) =>
                    (v || '').length <= 200 ||
                ...
                row-height="3"
                hint="Trata de ser breve y consiso"
                label="Descripción"
              ></v-textarea>
            </v-col>
          </v-row>
  ...
  computed: {
    ...
    stateBtn() {
      ...
      if (this.estructure.ars_name.length > 0) {
        if (this.estructure.ars_description.length > 0) {
      ...
      if (this.editedIndex === -1) {
        this.$http
          .post("areas/register/areacampus",{
            ...
            "ars_description":this.estructure.ars_description,
